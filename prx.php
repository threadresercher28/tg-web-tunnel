<?php
ini_set('max_execution_time',30);ini_set('memory_limit','128M');ini_set('zlib.output_compression','Off');ini_set('output_buffering','Off');
if(function_exists('apache_setenv'))apache_setenv('no-gzip','1');
if(session_status()===PHP_SESSION_ACTIVE)session_write_close();
ini_set('display_errors','0');error_reporting(0);
while(ob_get_level())ob_end_clean();

define('MAX_REDIRECTS',5);define('CONNECT_TIMEOUT',10);define('TIMEOUT',30);define('MAX_POST_SIZE',10485760); // 10 MB
...
function isUrlSafe($url){$parsed=parse_url($url);if(!isset($parsed['host']))return false;
    $host=strtolower($parsed['host']);
    // Блокируем любой IP-адрес в URL (доменное имя обязательно)
    if(filter_var($host,FILTER_VALIDATE_IP))return false;
    // Только точное совпадение с разрешёнными хостами (поддомены запрещены)
    if(!in_array($host,ALLOWED_HOSTS,true))return false;
    // Дополнительно: проверяем, что резолв не возвращает IP вне разрешённых диапазонов
    $ips=[];
    $dns4=@dns_get_record($host,DNS_A);$dns6=@dns_get_record($host,DNS_AAAA);
    ...
    foreach($ips as$ip){$allowed=false;foreach(ALLOWED_IP_RANGES as$range){if(ipInRange($ip,$range)){$allowed=true;break;}}if(!$allowed)return false;}
    return true;
}
...
function normalizeUrl($url){$url=trim($url);if(empty($url)||strlen($url)>2048)return false;
    if(!preg_match('~^(?:f|ht)tps?://~i',$url)){$url='https://'.$url;} // только HTTPS для Telegram
    $parsed=parse_url($url);if(!$parsed||!isset($parsed['host']))return false;
    $scheme='https'; // принудительно HTTPS
    $host=$parsed['host'];$port=isset($parsed['port'])?':'.$parsed['port']:'';
...
function sendError($code,$genericMessage=''){if(headers_sent())return;if(ob_get_level())ob_clean();
...
function executeCurlRequest($url,$method,$headers,$body,$redirectCount){
    // Ограничиваем методы
    if(!in_array($method,['GET','POST','PUT','DELETE','PATCH'])) sendError(405);
    $ch=curl_init();
    $curlHeaders=[];
    foreach($headers as$name=>$value){
        $n=strtolower($name);
        if(in_array($n,['host','connection','content-length','expect','transfer-encoding','proxy-connection'])) continue;
        $curlHeaders[]="$name: $value";
    }
    // Заменяем User-Agent клиента на наш (безопасность)
    $hasUA=false;
    foreach($curlHeaders as$i=>$h){if(stripos($h,'user-agent:')===0){$curlHeaders[$i]='User-Agent: '.USER_AGENT;$hasUA=true;break;}}
    if(!$hasUA) $curlHeaders[]='User-Agent: '.USER_AGENT;
    ...
    $options=[
        ...
        CURLOPT_ENCODING=>'' // автоматическая обработка Content-Encoding
    ];
    if($body&&in_array($method,['POST','PUT','PATCH','DELETE'])){
        if(strlen($body)>MAX_POST_SIZE) sendError(413);
        $options[CURLOPT_POSTFIELDS]=$body;
    }
    $pinData=resolveAndPinIp($url);
    if($pinData!==null){list($hostname,$port,$ip)=$pinData;$options[CURLOPT_RESOLVE]=["$hostname:$port:$ip"];}
    ...
    $headerFunc=function($ch,$data)use(&$headerLines,&$httpCode,&$isRedirect,&$locationUrl,&$headersSentToClient,$redirectCount){
        ...
        if($line===''){
            if($isRedirect){
                foreach($headerLines as$h){if(preg_match('/^Location:\s*(.*)$/i',$h,$loc)){$locationUrl=trim($loc[1]);break;}}
                return strlen($data);
            }
            if(!$headersSentToClient){
                ...
                foreach($headerLines as$h){
                    ...
                    if(count($parts)==2){
                        ...
                        $nLow=strtolower($n);
                        if(in_array($nLow,['transfer-encoding','connection','keep-alive','proxy-connection','content-security-policy'])) continue;
                        header("$n: $v",false);
                    }
                }
    ...
    curl_exec($ch);
    $info=curl_getinfo($ch);$error=curl_error($ch);curl_close($ch);

    if($error){
        if(!$headersSentToClient) sendError(502);
        return;
    }
    if($isRedirect&&$locationUrl){
        if(ob_get_level())ob_clean();
        $newUrl=resolveUrl($locationUrl,$url);
        if(!isUrlSafe($newUrl)) sendError(403);
        followRedirects($newUrl,$redirectCount+1);
    }
}

function followRedirects($url,$redirectCount){
    if($redirectCount>MAX_REDIRECTS) sendError(508);
    $method=$_SERVER['REQUEST_METHOD'];
    $headers=getClientHeaders();
    $body=file_get_contents('php://input');
    if(strlen($body)>MAX_POST_SIZE) sendError(413);
    executeCurlRequest($url,$method,$headers,$body,$redirectCount);
}

function runProxy(){
    $target=$_REQUEST['url']??null;
    if(!$target) sendError(400);
    if(strlen($target)>2048) sendError(400);
    $target=normalizeUrl($target);
    if(!$target) sendError(400);
    if(!isUrlSafe($target)) sendError(403);
